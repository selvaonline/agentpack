// agentpack — the built-in dev UI: live agent network + chat, zero build
// step, served at GET /. Vanilla JS over the same SSE contract the eval
// harness uses.
import type { AgentPack } from "./types.js";

export function devUiHtml(pack: AgentPack): string {
  return `<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="utf-8"/>
<meta name="viewport" content="width=device-width,initial-scale=1"/>
<title>${esc(pack.name)} — agentpack</title>
...
.querybar{display:flex;gap:10px;margin-bottom:20px}
...
<script>
const els = {};
for (const id of ["q","go","suprow","cols","feed","answer","hops","timer","stats","netsub"]) els[id] = document.getElementById(id);
let net = null, nodeEls = {}, hopCount = 0, t0 = 0, timerIv = null;
...
async function loadNetwork() {
  ...
  const supEl = mkNode(sup.id, "sup"); els.suprow.appendChild(supEl);
...
async function run() {
  ...
  const threadId = sessionStorage.getItem("agentpack-thread") || undefined;
  const r = await fetch("/api/run", { method:"POST", headers:{"Content-Type":"application/json"},
    body: JSON.stringify({ query, threadId }) });
  const { runId, threadId: tid } = await r.json();
  if (tid) sessionStorage.setItem("agentpack-thread", tid);
  ...
  es.onmessage = (m) => {
    ...
    if (ev.kind === "run_finished") {
      es.close(); clearInterval(timerIv); els.go.disabled = false;
      for (const el of Object.values(nodeEls)) el.classList.remove("active");
